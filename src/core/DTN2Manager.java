/* 
 * Copyright 2008 Teemu Karkkainen
...
public class DTN2Manager {
	...
	public static void setup(World world) {
		...
		// XXX: read config file from settings 
		...
		while (s!=null) {
			...
			if (attrs.length==5 && !s.startsWith("#")) {
				...
				// XXX: Could add some window etc for controlling the CLAs
				CLAParser p;
				p = new CLAParser(dtnd_host, dtnd_port, "ONE");
				/* XXX: We assume here that the DTN2Events class has been loaded
				  and instantiated by the dynamic class loader or whatnot */
				DTN2Events.ParserHandler ph = 
					DTN2Manager.events.getParserHandler(nodeID, dtnd_host,
							console_port); 
				p.setListener(ph);
				//p.setDebugStream(System.out); // XXX: DEBUG
